import { DropdownMenu3 } from '@/partials/dropdown-menu/dropdown-menu-3';
import { DropdownMenu6 } from '@/partials/dropdown-menu/dropdown-menu-6';
import { EllipsisVertical } from 'lucide-react';
import { Link } from 'react-router-dom';
import { toAbsoluteUrl } from '@/lib/helpers';
import { Button } from '@/components/ui/button';
import {
  Card,
  CardContent,
  CardFooter,
  CardHeader,
  CardTitle,
} from '@/components/ui/card';

const Policies = () => {
  const items = [
    {
      avatar: 'Medicare-Academy-Policies.png',
      check: 'plus.svg',
      name: 'Medicare Academy',
      connections: 10,
      connected: false,
    },
    {
      avatar: 'Medicare-Insurance-Policies.png',
      check: 'completed.svg',
      name: 'Medicare Insurance',
      connections: 10,
      connected: true,
    },
    {
      avatar: 'Medicare-Academy-Policies.png',
      check: 'plus.svg',
      name: 'Medicare Academy',
      connections: 10,
      connected: false,
    },
    {
      avatar: 'Medicare-Insurance-Policies.png',
      check: 'completed.svg',
      name: 'Medicare Insurance',
      connections: 10,
      connected: true,
    },
  ];

  const renderItem = (item, index) => {
    return (
      <div key={index} className="flex items-center gap-2">
        <div className="flex items-center grow gap-2.5">
          <img
            src={toAbsoluteUrl(`/media/avatars/${item.avatar}`)}
            className="rounded-full size-9 shrink-0"
            alt="image"
          />

          <div className="flex flex-col">
            <Link
              to="#"
              className="text-sm font-semibold text-mono hover:text-primary-active mb-px"
            >
              {item.name}
            </Link>
            <span className="text-xs font-semibold text-secondary-foreground">
              {item.connections} Sold
            </span>
          </div>
        </div>
        <DropdownMenu3
          trigger={
            // <Button variant="ghost" mode="icon">
              <img
                src={toAbsoluteUrl(`/media/avatars/${item.check}`)}
                className="rounded-full size-9 shrink-0"
                alt="image"
              />
            // </Button>
          }
        />
      </div>
    );
  };

  return (
    <Card>
      <CardHeader className="gap-2">
        <CardTitle>Top 10 Policies</CardTitle>
        <DropdownMenu6
          trigger={
            <Button variant="ghost" mode="icon">
              <EllipsisVertical />
            </Button>
          }
        />
      </CardHeader>
      <CardContent>
        <div className="flex flex-col gap-2 lg:gap-5">
          {items.map((item, index) => {
            return renderItem(item, index);
          })}
        </div>
      </CardContent>
      <CardFooter className="justify-center">
        <Button mode="link" underlined="dashed" asChild>
          <Link to="/public-profile/network">View All</Link>
        </Button>
      </CardFooter>
    </Card>
  );
};

export { Policies };
